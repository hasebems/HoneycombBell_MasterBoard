...
//-------------------------------------------------------------------------
int MBR3110_prepareWritingConfig( int number, unsigned char i2cAdrs )
{
  unsigned char i2cdata[2];
  delay(15);
  i2cdata[0] = CTRL_CMD;
  ...
  write_i2cDevice(i2cAdrs,i2cdata,2);
  delay(900);
  return MBR3110_writeConfig(number,i2cAdrs);
}
//-------------------------------------------------------------------------
int MBR3110_setup( int number )
{
  while(1){
    ...
    int cnfgerr = MBR3110_prepareWritingConfig(number, CAP_SENSE_ADDRESS_ORG);

    if ( cnfgerr != 0 ){
      //  if err then change I2C Address
      //    for rewriting config to current device
      cnfgerr = MBR3110_prepareWritingConfig(number, tI2cAdrs[number]);
    }
    if ( cnfgerr != 0 ){
      //  if mistaking 
      cnfgerr = MBR3110_prepareWritingConfig(number, tI2cAdrs[0]);     
    }
    if ( cnfgerr != 0 ){
      //  if mistaking 
      cnfgerr = MBR3110_prepareWritingConfig(number, tI2cAdrs[1]);     
    }
